  ...
  return `
    <a class="feed-card" href="recipe.html?id=${encodeURIComponent(r.id)}">
      ...
      <div class="fc-body">
        ...
        <h3>${r.recipe_name}</h3>
        <span class="fc-author">by. ${r.author || '익명의 요리사'}</span>
        <div class="fc-meta">
...
async function loadFeed() {
  try {
    ...
    if (supabase) {
      ...
      recipes = [...local, ...base.map((r, i) => ({ ...r, id: 'seed-' + i }))];
    }

    if (recipes.length === 0) {
      grid.innerHTML = '<p class="recipe-loading">아직 등록된 레시피가 없어요. 첫 레시피를 올려보세요!</p>';
      return;
    }
    grid.innerHTML = recipes.map(cardTemplate).join('');
  } catch (err) {
    console.error(err);
