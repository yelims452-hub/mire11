...
async function signUp({ username, email, password, birthdate, bio }) {
  ...
  // 이유: 이메일 확인이 필요한 프로젝트 설정에서는 signUp() 직후 로그인 세션이 없어
  // auth.uid()가 비어있고, 클라이언트에서 바로 profiles.insert()를 하면 RLS(본인만 생성 가능
  // 정책, auth.uid() = id)에 막혀 항상 실패한다. 그래서 신원 확인(userId<->email 대조)을
  // 거친 서버가 service_role 권한으로 대신 생성한다.
  const profileRes = await fetch('/api/create-profile', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ userId, username, email, birthdate, bio }),
  });
