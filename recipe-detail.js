...
function commentFormTemplate(loggedInUsername) {
  if (loggedInUsername) {
    return `
      <form id="comment-form" class="comment-form">
        <span class="comment-as-badge">@${escapeHtml(loggedInUsername)}</span>
  ...
  return `
    <div class="comment-login-notice">
      ...
      <a class="button button-primary small" href="login.html">로그인하기 <span>→</span></a>
    </div>
  `;
}
...
async function main() {
  ...
  if (supabase && !recipeId.startsWith('seed-') && !recipeId.startsWith('local-')) {
    ...
    document.getElementById('comment-form')?.addEventListener('submit', (e) => {
      ...
      list.push({ author: loggedInUsername, content, created_at: new Date().toISOString() });
